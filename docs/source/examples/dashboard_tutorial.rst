...
This tutorial guides you through creating a professional Dash application. We will replicate the architecture of complex production apps (Sidebar, Filter Panel, KPIs, and Interactive Graphs) but apply a **"Friendly & Fresh"** design system to make it visually appealing.

.. note::
   This tutorial uses the built-in **Gapminder** dataset so you can run it immediately without downloading external CSV files. The architecture mirror's typical data science dashboards (e.g., medical or financial apps).

1. Design Philosophy
--------------------
Instead of the default styles or heavy dark themes, we will use a **"Soft UI"** approach:
*   **Colors**: Pastels and soft blues for less eye strain.
*   **Shapes**: Rounded corners (`borderRadius`) for a friendly feel.
*   **Depth**: Subtle shadows (`boxShadow`) instead of harsh borders.
*   **Space**: Generous padding to let the data breathe.
...
3. The Complete App Code
------------------------
Here is the full application. Save this as `modern_app.py` and run it.
...
Sidebar Layout
^^^^^^^^^^^^^^
The ``SIDEBAR_STYLE`` and ``CONTENT_STYLE`` dictionaries create the layout where the sidebar is fixed to the left and content flows to the right.
...
Friendly Cards
^^^^^^^^^^^^^^
We use a reusable ``CARD_STYLE`` dictionary. The key to the "Modern" look is:
1.  **Border Radius**: ``'borderRadius': '12px'`` shortens the corners.
2.  **Shadow**: ``'boxShadow'`` adds depth without heavy borders.
3.  **Whitespace**: Generous ``'padding': '24px'``.

Clean Graphs
^^^^^^^^^^^^
Plotly graphs often have grey backgrounds by default. We clean them up to match our theme:
...
This ensures the graphs blend seamlessly into our white cards.
